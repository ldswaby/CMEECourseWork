...
def main(argv):
    ...
    # Create list of best alignments
    for i in range(l1):
        z = calculate_score(s1, s2, l1, l2, i)
        if z == my_best_score:
            my_best_aligns.append("-" * i + s2)

    # Create SeqRecords for output fasta
    outrecs = []

    for no, alignmt in enumerate(my_best_aligns):
        outrec = SeqRecord(
            Seq(alignmt),
            id=f"Seq1: Alignment {no}",
    ...
    outrecs.append(largeseq)

    # Write to fasta
    SeqIO.write(outrecs, '../Results/group_better_algmt.fa', 'fasta')
